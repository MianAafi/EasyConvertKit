...
import "../../css/CompleteConversion.css";
...
function CompleteConversion() {
    ...
    const handleUploadDropbox = (service) => {
        if (file.length + filePathList.length >= maxFiles) {
            Swal.fire(
                "Warning",
        ...
        instance
            ...
            .then((response) => {
                if (response.data.error === "File not accessable") {
                    // Handle the case where the file is restricted and not accessible
                    Swal.fire(
                        "Error",
                        "This URL is invalid! Try Again.",
                        "error"
                    );
                    handleClose();
                    return;
                ...
                if (parseFloat(fileSize) > maxMB) {
                    Swal.fire(
                        "Warning",
                        "Each File can't be more than 10 MB",
                        "warning"
                    );
                    handleClose();
                    return;
    ...
    const handleUploadGoogleDrive = (service) => {
        ...
        instance
            ...
            .then((response) => {
                if (response.data.error === "File not accessable") {
                    // Handle the case where the file is restricted and not accessible
                    Swal.fire("Error", "The file is not accessible.", "error");
                    handleClose();
                    return;
                }
                const filePath = response.data.filePath;
    ...
    const { getRootProps, getInputProps, inputRef } = useDropzone({
        ...
        onDrop: (acceptedFiles) => {
            if (
                acceptedFiles.length + filePathList.length + file.length >
                    maxFiles ||
                file.length >= maxFiles ||
                filePathList.length >= maxFiles
            ) {
                Swal.fire(
                    "Warning",
                    "You can't select more than 10 Files",
                    "warning"
                );
                return;
            }
            ...
            for (let i = 0; i < acceptedFiles.length; i++) {
                const singleFileSizeInMB = acceptedFiles[i].size / 1000000;
                if (singleFileSizeInMB > maxMB) {
                    Swal.fire(
                        "Warning",
        ...
        <>
            <div className="documentPage">
                ...
                <p className="heading-p text-center">
                    Convert documents, images - 50+ <br /> formats supported
                </p>
                {file.length === 0 && filePathList.length === 0 && (
                    <div className="Container UploadFile ">
                        ...
                        <p className="convert-belowP">
                            Up to 10 files daily. (10 files remaining today), 10
                            MB each
                        </p>
                    </div>
                )}
...
export default CompleteConversion;
